...
class GeneralizedRelationalConv(MessagePassing):
    ...
    def __init__(self, input_dim, output_dim, num_relation, query_input_dim, message_func="distmult",
        ...
        if dependent:
            ...
            if not self.project_relations:
                ...
                # print("===========")
                # print(self.relation.weight.shape)
                # print(self.relation.weight.grad)
                # exit()
            else:
    ...
    def forward(self, input, query, boundary, edge_index, edge_type, size, edge_weight=None, h_index=None, relation_embeddings=None):
    ...
    def message_and_aggregate(self, edge_index, input, relation, boundary, edge_type, edge_weight, index, dim_size):
        ...
        if edge_index.numel() == 0:
            # fallback: no messages to aggregate → return just boundary
            # print("======================")
            # print("edge_index is empty")
            # print(boundary.transpose(0, 1).shape)
            # print(boundary.transpose(0, 1))
            return boundary
