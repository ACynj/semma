...
def get_synset_from_offset(offset_id):
    offset_id = int(offset_id)
    for synset in wn.all_synsets():
        if synset.offset() == offset_id:
            return synset
    return None
...
@torch.no_grad()
def test(cfg, model, test_data, device, logger, filtered_data=None, return_metrics=False, is_test=True):
    world_size = util.get_world_size()
    rank = util.get_rank()
    ...
    for batch in test_loader:
        ...
        if(flags.harder_setting == True):
            h_pred = model(test_data, h_batch, is_tail=True)
        else:
        ...
        t_ranking = tasks.compute_ranking(t_pred, pos_t_index, t_mask) # pos_t_index is the correct answer
...
if __name__ == "__main__":
    ...
    if util.get_rank() == 0:
        ...
        with open(os.path.join(mydir, "flags.yaml"), 'r') as file:
            for line_here in file:
                logger.warning(line_here.strip())
        logger.warning(separator)
    ...
    if task_name == "InductiveInference":
        ...
        if "ILPC" in cfg.dataset['class'] or "Ingram" in cfg.dataset['class']:
            ...
            inf_triplets = torch.cat([test_data.edge_index, test_data.edge_type.unsqueeze(0)]).t()
            test_triplets = torch.cat([test_data.target_edge_index, test_data.target_edge_type.unsqueeze(0)]).t()
        ...
        inf_triplets = torch.cat([train_data.edge_index, train_data.edge_type.unsqueeze(0)]).t()
        test_triplets = torch.cat([test_data.target_edge_index, test_data.target_edge_type.unsqueeze(0)]).t()
    ...
    if(flags.eval_on_valid == True):
        if util.get_rank() == 0:
            logger.warning(separator)
            logger.warning("Evaluate on valid")
        test(cfg, model, valid_data, filtered_data=val_filtered_data, device=device, logger=logger, is_test=False)
    if util.get_rank() == 0:
        logger.warning(separator)
