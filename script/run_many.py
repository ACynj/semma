...
if __name__ == "__main__":
    ...
    for graph in datasets:
        ...
        for i in range(args.repeats):
            ...
            vars = parse.detect_variables(args.config, root=mydir)
            parser = argparse.ArgumentParser()
            for var in vars:
                parser.add_argument("--%s" % var)
            vars = parser.parse_known_args(unparsed)[0]
            vars = {k: parse.literal_eval(v) for k, v in vars._get_kwargs()}
            ...
            cfg = parse.load_config(args.config, context=vars, root=mydir)
            ...
            # cfg = parse.load_config(args.config, context=vars)
            # working_dir = util.create_working_directory(cfg)
