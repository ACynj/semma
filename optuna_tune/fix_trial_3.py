...
def find_evaluation_logs_for_trial3():
    """查找Trial 3的评估日志文件（第四次实验）"""
    eval_dir = "/T20030104/ynj/semma/v3_vip_output/Ultra"
    if not os.path.exists(eval_dir):
        return {}
    
    # Trial 3的预训练完成时间（用于确认评估日志）
    pretrain_checkpoint = "/T20030104/ynj/semma/output/Ultra/JointDataset/2025-11-08-07-29-58/model_epoch_10.pth"
...
def fix_trial_3(result_json_path=None):
    """修复Trial 3的结果（第四次实验）
    
    Args:
        result_json_path: 可选，包含trial 3结果的JSON文件路径
    ...
    trial_3 = study.trials[3]
    print(f"\n📋 Trial 3当前状态:")
    print(f"  状态: {trial_3.state}")
    print(f"  值: {trial_3.value}")
    print(f"  参数: {trial_3.params}")
    
    # 确认这是第四次实验
    print(f"\n🔍 确认这是第四次实验:")
    print(f"  参数: similarity_threshold_init={trial_3.params['similarity_threshold_init']}, "
          f"enhancement_strength_init={trial_3.params['enhancement_strength_init']}")
    
    # 检查预训练目录
    pretrain_dir = "/T20030104/ynj/semma/output/Ultra/JointDataset/2025-11-08-07-29-58"
    ...
    if trial_3.state == optuna.trial.TrialState.COMPLETE and trial_3.value != float('inf'):
        print("\n⚠ Trial 3已经完成且有值，将覆盖现有结果")
    ...
    # 如果参数未知，从现有trial获取
    if params is None:
        params = trial_3.params
        print(f"\n📝 从数据库读取的参数: {params}")
    
    # 4. 从评估日志中提取结果（如果还没有）
    if eval_results is None:
        print(f"\n📊 尝试从评估日志中提取指标（Trial 3的评估）...")
        eval_results = find_evaluation_logs_for_trial3()
    ...
    try:
        ...
        # 获取trial_id
        cursor.execute("SELECT trial_id FROM trials WHERE number = 3")
    ...
    # 7. 保存trial结果文件
    print(f"\n📁 保存trial结果文件...")
    trial_dir = "/T20030104/ynj/semma/optuna_tune/trials/trial_3"
    ...
    if len(study.trials) > 3:
        trial_3_updated = study.trials[3]
        print(f"  Trial 3新状态:")
        print(f"    状态: {trial_3_updated.state}")
        print(f"    值: {trial_3_updated.value:.4f} (对应分数: {-trial_3_updated.value:.4f})")
        print(f"    参数: {trial_3_updated.params}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='修复Trial 3的结果（第四次实验）')
    parser.add_argument('--result_json', type=str, default=None,
                       help='包含trial 3结果的JSON文件路径（格式: {"params": {...}, "eval_results": {...}}）')
    args = parser.parse_args()
    
    success = fix_trial_3(result_json_path=args.result_json)
