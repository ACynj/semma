...
def find_evaluation_logs_for_trial2():
    """查找Trial 2的评估日志文件"""
...
def fix_trial_2(result_json_path=None):
    """修复Trial 2的结果
    
    Args:
        result_json_path: 可选，包含trial 2结果的JSON文件路径
    ...
    print(f"  值: {trial_2.value}")
    print(f"  参数: {trial_2.params}")
    
    if trial_2.state == optuna.trial.TrialState.COMPLETE and trial_2.value != float('inf'):
        print("\n⚠ Trial 2已经完成且有值，将覆盖现有结果")
    ...
    if eval_results is None:
        print(f"\n📊 尝试从评估日志中提取指标...")
        eval_results = find_evaluation_logs_for_trial2()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='修复Trial 2的结果')
    parser.add_argument('--result_json', type=str, default=None,
                       help='包含trial 2结果的JSON文件路径（格式: {"params": {...}, "eval_results": {...}}）')
