# 快速测试命令

## 🚀 最简单的方式（3步）

### 步骤1: 修改flags.yaml
```bash
# 编辑flags.yaml，将第一行改为：
run: EnhancedUltra
```

### 步骤2: 激活环境并运行测试
```bash
conda activate semma

# 使用现有checkpoint测试（推荐）
python script/run.py \
    -c config/transductive/inference.yaml \
    --dataset FB15k237 \
    --epochs 0 \
    --bpe null \
    --ckpt ckpts/semma.pth \
    --gpus [0]
```

### 步骤3: 查看结果
在输出中查找以下指标：
- `mrr: 0.xxx` - 这是最重要的指标
- `hits@1: 0.xxx`
- `hits@10: 0.xxx`

## 📊 对比测试（推荐）

使用提供的测试脚本自动对比：

```bash
conda activate semma

# 自动对比基线和增强版本
bash test_similarity_enhancement.sh FB15k237 ckpts/semma.pth [0]
```

## 🔍 手动对比测试

### 1. 测试基线（SEMMA）
```bash
# 确保flags.yaml中: run: semma
conda activate semma
python script/run.py \
    -c config/transductive/inference.yaml \
    --dataset FB15k237 \
    --epochs 0 --bpe null \
    --ckpt ckpts/semma.pth \
    --gpus [0] \
    > baseline.log 2>&1
```

### 2. 测试增强版（EnhancedUltra）
```bash
# 修改flags.yaml: run: EnhancedUltra
conda activate semma
python script/run.py \
    -c config/transductive/inference.yaml \
    --dataset FB15k237 \
    --epochs 0 --bpe null \
    --ckpt ckpts/semma.pth \
    --gpus [0] \
    > enhanced.log 2>&1
```

### 3. 对比结果
```bash
echo "=== 基线 ==="
grep "mrr:" baseline.log
grep "hits@1:" baseline.log
grep "hits@10:" baseline.log

echo "=== 增强 ==="
grep "mrr:" enhanced.log
grep "hits@1:" enhanced.log
grep "hits@10:" enhanced.log
```

## 📝 常用命令模板

### 测试不同数据集
```bash
# FB15k237
python script/run.py -c config/transductive/inference.yaml \
    --dataset FB15k237 --epochs 0 --bpe null \
    --ckpt ckpts/semma.pth --gpus [0]

# WN18RR
python script/run.py -c config/transductive/inference.yaml \
    --dataset WN18RR --epochs 0 --bpe null \
    --ckpt ckpts/semma.pth --gpus [0]
```

### 修改flags.yaml的快捷方式
```bash
# 切换到EnhancedUltra
sed -i 's/run: semma/run: EnhancedUltra/' flags.yaml

# 切换回semma
sed -i 's/run: EnhancedUltra/run: semma/' flags.yaml

# 查看当前设置
grep "^run:" flags.yaml
```

## ⚡ 一键测试脚本

```bash
#!/bin/bash
# 一键快速测试

# 备份并切换到EnhancedUltra
cp flags.yaml flags.yaml.bak
sed -i 's/run:.*/run: EnhancedUltra/' flags.yaml

# 运行测试
conda activate semma
python script/run.py \
    -c config/transductive/inference.yaml \
    --dataset FB15k237 \
    --epochs 0 --bpe null \
    --ckpt ckpts/semma.pth \
    --gpus [0]

# 恢复
mv flags.yaml.bak flags.yaml
```

## 💡 提示

1. **使用现有checkpoint**: 不需要重新训练，可以直接测试
2. **注意flags.yaml**: 确保设置正确（`run: EnhancedUltra`）
3. **查看完整日志**: 如果输出被重定向，检查日志文件
4. **GPU内存**: 如果内存不足，可以减小batch_size



